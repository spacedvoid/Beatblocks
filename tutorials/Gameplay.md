...
아래와 같은 "타임라인"이 있다고 합시다.

![Timeline1](./images/Gameplay_Timeline_1.png)

파란 선은 50m마다 판정이 처리되기 시작하는 시간대입니다.
...
즉, 판정이 실제로 초록 선에 입력되었음에도 Beatblocks는 직후의 빨간 선에서 입력이 있었다고 가정합니다.
...
실제 입력(녹색 선)은 실제 타이밍 직후에 들어와 보기에는 PERFECT 판정으로 처리되어야 한다고 생각되지만,
주황색 영역 안에서 들어오는 입력은 빨간색 선에서, 실제 입력은 노란색 선에서 처리되기 때문에 이 판정은 GREAT로 처리됩니다.
...
하지만 이 앞뒤 틱 사이에 또다른 입력을 필요로 한다면 이는 무시되어, "애매한 판정"이 적용되지 않습니다.
